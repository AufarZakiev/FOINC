...
| `socket`, `urllib.request`, `urllib.error`, `http.client`, `ftplib`, `smtplib`, `telnetlib`, `poplib`, `imaplib`, `nntplib`, `xmlrpc.client`, `subprocess` | Install a stub `types.ModuleType` instance into `sys.modules` under each name. The stub defines `__getattr__(name)` that raises `RuntimeError("network access is disabled")` for network modules and `RuntimeError("subprocess execution is disabled")` for `subprocess`. **Import itself succeeds** (because `sys.modules[name]` is a real module object, not `None`); the `RuntimeError` fires on first attribute access, producing a traceback with the sandbox's standard message. This is the intentional choice over `sys.modules[name] = None`, which would raise `ImportError` at `import` time with a generic message. | `import socket` succeeds; `import subprocess` succeeds; etc. | `socket.socket(...)`, `subprocess.run(...)`, `urllib.request.urlopen(...)`, etc. raise `RuntimeError`. |
| `urllib.parse` | Not blocked. | Normal import. | Normal use. |
| `os` subprocess surface: `os.system`, `os.popen`, `os.execv`, `os.execvp`, `os.execve`, `os.execl`, `os.execlp`, `os.execle`, `os.execvpe`, `os.spawnl`, `os.spawnle`, `os.spawnlp`, `os.spawnlpe`, `os.spawnv`, `os.spawnve`, `os.spawnvp`, `os.spawnvpe`, `os.posix_spawn`, `os.posix_spawnp`, `os.forkpty`, `os.fork` | For each name present on the live `os` module object, perform `setattr(os, "<name>", stub)` where `stub` raises `RuntimeError("subprocess execution is disabled")`. Use `setattr`, NOT `delattr` — user tracebacks must uniformly read `RuntimeError("subprocess execution is disabled")` instead of a mix of `AttributeError` and `RuntimeError`. Names not present on the current Pyodide build's `os` (Pyodide's WASM omits `fork`/`exec` already) are skipped silently; the guarantee is that if the name resolves, it raises. | — | `os.system(...)`, `os.popen(...)`, `os.execv(...)`, `os.fork(...)`, etc. raise `RuntimeError`. |
...
**Failure mode of the sandbox itself.** The mechanisms above are pure Python dict assignments (`sys.modules[name] = stub_module`) and `setattr` on well-known module objects (`pyodide.http`, `pyodide.code`, `pyodide.ffi`, `js`, `os`). On a functioning Pyodide instance these operations cannot raise — the modules are standard, the dict is writable, and the attribute names are all valid Python identifiers. If any of them does raise, the Pyodide instance is assumed broken: the runtime catches the exception in step 2-5 and surfaces it via the `Running | Sandbox application (steps 2-5) fails | Idle` transition defined in the State Machine. User code never runs in that case.
...
**Not blocked, intentionally.** `pathlib`, `ctypes`, `builtins.open`, the `os` module minus the subprocess surface enumerated above, and the full Python stdlib for CPU/in-memory work (math, itertools, collections, json, csv, re, statistics, …) remain available to user scripts. Filesystem isolation is handled by the worker boundary, not by removing modules.
